...
- Inclusion: Peer-reviewed, English-language, empirical or review, 2018-2025
- Exclusion: K-12, non-STEM, editorials
- Date Range: 2018-2025

**Coverage Assessment**: Strong coverage of English-language literature. Moderate coverage of regional-language sources (Airiti). Gap: limited grey literature from Taiwan MOE reports.
...
| `abstract` | object | `{english: string}` |
| `authors` | list[Author] | Author information with CRediT roles |
| `keywords` | object | `{en: list[string]}` (3-6 keywords) |
...
| `tool_artifact_intake_declaration` | object | Passport-level intake decision for the Tool Paper route (v3.4.0). `status` ∈ `{artifacts_declared, no_artifacts_declared, legacy_unknown}` + `declared_at` + `declared_by: scholar`. Set by the Stage 1 intake layer (never the manifest writers). **Fail-closed but ROUTE-CONDITIONAL** (deliberate deviation from #260's unconditional D7): required only when paper_type is Tool Paper (Design & Evaluation) OR any artifact field is present (`tool_artifact_provenance[]` non-empty, or any manifest claim carrying `planned_artifact_ids[]`); every other paper MUST NOT gain the key — a non-tool run stays byte-identical to a pre-v3.4.0 run. AP-INV-4 enforces declaration↔provenance symmetry. See [Tool-Artifact Provenance Intake (Tool Paper route)](#tool-artifact-provenance-intake-tool-paper-route) below. |
| `tool_artifact_provenance` | list[object] | Optional scholar-entered ledger of NON-BIBLIOGRAPHIC artifacts (code, architecture diagrams, rule inventories, screenshots, logs, usability data, simulation scenarios/outputs, protocol documents, process artifacts) backing Tool Paper claims (v3.4.0). Each entry conforms to [`shared/contracts/passport/tool_artifact_provenance_entry.schema.json`](contracts/passport/tool_artifact_provenance_entry.schema.json) — `artifact_id` (passport-flat, frozen at intake) + `artifact_kind` + `locator` + `produced_by` + `content_units[]` + `negative_findings[]` + `known_limitations[]`. Named `tool_artifact_provenance` (not `artifact_provenance`) to stay distinct from the pre-existing `artifact_provenance` field in `preregistration_artifact.schema.json`. ARS does not build artifacts, auto-fill provenance, or judge artifact quality/utility. Joined from claims via `claim_intent_manifest.planned_artifact_ids[]`. Gated at the integrity verification stage (Stage 2.5/4.5, Phase C5). |
| `artifact_alignment_results` | list[object] | Optional aggregate of claim→artifact alignment verdicts (v3.4.0) — the FIFTH ref_slug-less claim-finding aggregate. Each entry conforms to [`shared/contracts/passport/artifact_alignment_result.schema.json`](contracts/passport/artifact_alignment_result.schema.json); `alignment_verdict` ∈ `{ALIGNED, OVERSTATED, NOT_SUPPORTED_BY_PROVENANCE, PROVENANCE_INSUFFICIENT}` (same vocabulary as the experiment path, rule_version AA-v1). **Produced by the integrity verification agent AT the gate** (Phase C5), NOT by the claim-alignment audit agent. AA-INV-1/2 enforce id-uniqueness + reference resolution. Carried forward by `pipeline_orchestrator_agent`'s aggregate hand-off. |
...
### Tool-Artifact Provenance Intake (Tool Paper route)

Schema 9 gains the **intake + alignment** layer for non-bibliographic artifacts — the primary evidence kind for Tool Papers (Design & Evaluation), whose subject is a built tool/artifact. Mirrors #260's experiment layer shape-for-shape, with one deliberate deviation called out inline. ARS keeps artifact construction outside the pipeline: the scholar builds or holds the artifact and declares it at intake. This extension records disclosure and lets manuscript claims be audited against declared provenance. It does **not** build artifacts, judge artifact quality or utility, auto-fill provenance, or require artifact provenance for non-Tool-Paper pipelines.

**Three additions** (all under the Optional-Fields table above):

1. `tool_artifact_intake_declaration` (passport-level object) — the Stage 1 intake decision, set by the intake/orchestrator layer (never by the three manifest writers):

   ```yaml
   tool_artifact_intake_declaration:
     status: artifacts_declared            # | no_artifacts_declared | legacy_unknown
     declared_at: "2026-09-22T10:00:00Z"
     declared_by: scholar                  # always scholar — an intake decision, not an agent emission
   ```

   **Route-conditional fail-closed (deliberate deviation from #260 D7).** The declaration is REQUIRED only when the run is on the Tool Paper route — paper_type is Tool Paper (Design & Evaluation) — OR any artifact field is present (`tool_artifact_provenance[]` non-empty, or any manifest claim carrying `planned_artifact_ids[]`). For every other paper the key MUST NOT be written: a pre-v3.4.0 run and a non-tool v3.4.0 run are byte-identical (the #260 experiment declaration is unconditional; making the artifact declaration unconditional too would perturb every existing pipeline). On the route, absence is a gate FAIL (Phase C5); `artifacts_declared` with empty provenance is a FAIL; `no_artifacts_declared` with artifact claims in the manifest is a FAIL.

2. `tool_artifact_provenance[]` (scholar-entered list) — one [`tool_artifact_provenance_entry.schema.json`](contracts/passport/tool_artifact_provenance_entry.schema.json) per artifact: `artifact_id` (passport-flat, FROZEN at intake) + `artifact_kind` (code_repository, architecture_diagram, rule_inventory, knowledge_base, screenshot, interface_mockup, timing_log, performance_log, usability_data, simulation_scenario, simulation_output, protocol_document, process_artifact, other) + `locator` + `produced_by` + `content_units[]` (what a claim can rest on — artifact_id alone is too coarse for "mean screening time 92 s") + `negative_findings[]` + `known_limitations[]` (both keys MUST be present; `[]` is well-formed; absent = malformed FAIL at Phase C5). Writers reference the frozen key space via `claim_intent_manifest.planned_artifact_ids[]`.

3. `artifact_alignment_results[]` (integrity-agent-produced list) — the fifth ref_slug-less claim-finding aggregate. Each [`artifact_alignment_result.schema.json`](contracts/passport/artifact_alignment_result.schema.json) row carries an `alignment_verdict` computed by the integrity verification agent **at the gate** (Stage 2.5/4.5, Phase C5), same vocabulary as the experiment path (`ALIGNED / OVERSTATED / NOT_SUPPORTED_BY_PROVENANCE / PROVENANCE_INSUFFICIENT`, rule_version AA-v1). A mixed-evidence claim (BOTH `planned_refs` AND `planned_artifact_ids`) gets one `claim_audit_results[]` row AND one `artifact_alignment_results[]` row; the gate combines them worst-verdict-wins.

**Invariants** (designed for lint enforcement in `scripts/check_claim_audit_consistency.py` — upstream; the script is not present in every checkout): AP-INV-1 (artifact_id unique/passport) · AP-INV-2 (planned_artifact_ids resolve; rename + forward-reference guard) · AP-INV-3 (artifact ids ⟹ intended_evidence_kind == 'artifact'; mixed literature+artifact allowed) · AP-INV-4 (route-conditional declaration↔provenance symmetry) · AA-INV-1 (finding_id unique) · AA-INV-2 (alignment row references resolve; dangling id = structural FAIL, never a verdict).

See [`academic-paper/docs/tool-paper-route.md`](../academic-paper/docs/tool-paper-route.md) for the route usage guide (invocation, sub-routes, worked example, model-tiering statement).
