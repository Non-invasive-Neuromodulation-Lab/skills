...
- For EN-only content: standard XeLaTeX or pdflatex pipeline is sufficient
...
- Field map: `body_font` → body text family/size; `caption` → figure/table caption
  font/size/placement/alignment; `line_spacing` → single/onehalf/double, or
...
- PDF: compile from LaTeX via `tectonic paper.tex` (see APA 7.0 LaTeX section)
```
...
**Abstract placement** (second language abstract):
```latex
\abstract{
...
└── Regional journals (TSSCI, etc.) -> generic article.cls
...
| `abstract_agent` | Abstract + Keywords | Markdown (EN) |
...
- **Receiving abstract_agent's Abstract**: the abstract is inserted as a standalone block; content is not modified
