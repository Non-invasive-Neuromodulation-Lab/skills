...
|---|---|---|---|
| `--venue=<v>` (v3.2, default) | Venue track | `venue_disclosure_policies.md` database (v2, 13 policy targets: ICLR / NeurIPS / Nature / Science / ACL / EMNLP + the #596 medical-publishing set — ICMJE / NEJM / The Lancet / JAMA / BMJ / PLOS / Frontiers) | Applicability/status bundle: `REQUIRED`, `ACTION_ONLY`, `NOT_REQUIRED`, or `UNKNOWN`, plus an explicit typed halt status when rendering cannot proceed |
...
For Frontiers, first record the
...
Never hard-code any specific AI product as the current tool. Never
...
- `venue_disclosure_policies.md` — policy database (v2: the 6 ML/NLP venues plus 7 medical-publishing policy targets incl. the ICMJE umbrella; see its Scope line)
