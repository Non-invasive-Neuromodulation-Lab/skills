...
**This is entirely optional.** All ARS skills work with the primary model alone. Cross-model verification is an additional layer for users who want higher confidence in integrity checks, devil's advocate challenges, and review judgments.
...
|-------|--------|----------|----------|
| Primary (session model) | _(inherited session model)_ | — | Primary model (default for all ARS skills) |
...
**Recommended cross-verification pair:** the inherited session model (primary) + a current-generation second-family verifier — Gemini 3.1 Pro (validated) or GPT-5.6 Sol (provisional; see the note below).

> The primary row deliberately names no version: the primary is always the session model, so the row cannot go stale on future model releases. Verifier IDs stay concrete because they are literal API strings the user must export. (`gpt-5.4` / `gpt-5.4-pro` remain accepted for existing setups.)
...
Using two models from different providers as primary+verifier is possible but not tested with ARS prompts.

## Setup Guide

### Prerequisites

You need API keys from at least one additional provider. ARS runs inside the host agent, so the session model is always available as the primary model.
...
- Primary model (primary) runs full Phase A-E verification as normal
...
5. Compare results: if the primary model said VERIFIED but cross-model said NOT_FOUND or MISMATCH, flag as `[CROSS-MODEL-DISAGREEMENT]`. Treat `NOT_SEARCHED` / ungrounded exactly as **not verified** — it never counts as agreement with a primary `VERIFIED`, and a sample that returns `NOT_SEARCHED` is surfaced for re-run or human review, never silently passed.
